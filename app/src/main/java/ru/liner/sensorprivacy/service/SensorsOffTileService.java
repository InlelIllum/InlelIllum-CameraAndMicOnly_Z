...
import ru.liner.sensorprivacy.SensorPrivacyController;
...
public class SensorsOffTileService extends TileService implements Shizuku.OnRequestPermissionResultListener, Shizuku.OnBinderReceivedListener, Shizuku.OnBinderDeadListener {
    ...
    private SensorPrivacyController privacyController;
    ...
    public void onCreate() {
        ...
        privacyController = new SensorPrivacyController(sensorPrivacyManager);
    ...
    public void onStartListening() {
        // Считываем реальное состояние системы, а не локальный кеш.
        // Это нужно, если состояние изменилось вне приложения
        // (например, через системные настройки).
        if (privacyController != null && privacyController.isAvailable()) {
            privacyEnabled = privacyController.isPrivacyEnabledForMicAndCamera();
        } else {
    ...
    private void setPrivacyEnabled(boolean enabled){
        ...
        if (shizukuState == ShizukuState.NORMAL) {
            if (!keyguardManager.isKeyguardLocked()) {
                // ВАЖНО: используем гранулярный API вместо глобального.
                // Блокируем ТОЛЬКО микрофон и камеру — акселерометр,
                // гироскоп, магнитометр, датчик тряски, шагомер и т.д.
                // остаются работать.
                boolean ok = privacyController != null
                        && privacyController.setPrivacyForMicAndCamera(enabled);
                if (ok) {
