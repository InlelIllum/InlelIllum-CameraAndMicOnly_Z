            ...
            public void handle(@NonNull Context context, @NonNull Intent intent) {
                Consumer.of(intent.getAction()).ifPresent(input -> {
                    if (input.equals(WINDOW_CHANGE_ACTION)) {
                        ...
                        try {
                            // Гранулярный режим: блокируем только микрофон и камеру,
                            // остальные датчики (тряска, поворот и т.д.) не трогаем.
                            if (privacyController != null) {
                                privacyController.setPrivacyForMicAndCamera(shouldBlock);
                            } else {
